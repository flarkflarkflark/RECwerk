(function ( w, d, PKAE ) {
	...
	var PKUI = function( app ) {
		...
		this.el.className += ' pk_app' + (app.isMobile ? ' pk_mob' : '') + (app.isDesktopApp ? ' pk_desktop' : ' pk_web');
		d.body.className += (d.body.className ? ' ' : '') + (app.isDesktopApp ? 'pk_desktop' : 'pk_web');
		...
		return [
			{
				...
				children : [
...

					{
						name: 'Recent Files',
						action: function ( e ) {
							var recent = app.getRecentFiles();
							var body = '<div class="pk_recent_list">';
							if (recent.length === 0) {
								body += '<p style="padding:20px; text-align:center;">No recent files found.</p>';
							} else {
								for (var i = 0; i < recent.length; i++) {
									body += '<div class="pk_recent_item" style="padding:10px; border-bottom:1px solid #335566; cursor:pointer;" data-index="' + i + '">' +
											'<div style="color:#99FF00; font-weight:bold;">' + recent[i].name + '</div>' +
											'<div style="color:#888; font-size:10px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">' + (recent[i].path || "No path available") + '</div>' +
											'</div>';
								}
							}
							body += '</div>';
							...
							new PKSimpleModal({
								...
								ondestroy: function(q) {
									app.ui.InteractionHandler.on = false;
									app.ui.KeyHandler.removeCallback('modalRecent');
								},
								setup: function(q) {
									var items = q.el_body.getElementsByClassName('pk_recent_item');
									for (var i = 0; i < items.length; i++) {
										items[i].onclick = function() {
											var index = this.getAttribute('data-index');
											var file = recent[index];
											
											// If in Electron, we can try to reopen directly
											if (window.process && window.process.versions && window.process.versions.electron && file.path) {
												const { ipcRenderer } = require('electron');
												ipcRenderer.invoke('read-file', file.path).then(data => {
													if (data) {
														// data is a Buffer (converted to Uint8Array)
														app.engine.LoadArrayBuffer(new Blob([data]), file.name, file.path);
														q.Destroy();
													}
												}).catch(err => {
													console.error("Failed to reopen file:", err);
													alert("Could not open '" + file.name + "'. The file might have been moved or deleted.\n\nPath: " + file.path);
													q.Destroy();
												});
												return;
											}

											alert("To reopen '" + file.name + "', please drag and drop it again or use 'Load from Computer'.");
											q.Destroy();
										};
									}
									app.ui.InteractionHandler.checkAndSet('modal');
								...
								buttons:[
									{
										title:'OPEN',
										callback: function( q ) {
											window.open ('pages/index-cache.html');
											q.Destroy ();
										}
									}
								],
	...
	function _makeUITopHeader ( menu_tree, UI ) {
		...
		function build_menus ( parent_el, tree_obj, level ) {
			for (var i = 0; i < tree_obj.length; ++i)
			{
				...
				{
					if (curr_obj.name === '---') {
						btn_container.className = 'pk_menu_sep';
					...
					if (curr_obj.type === 'file')
					{
						btn.className += ' pk_file';
					}
					...
					if (curr_obj.action)
					{
						...
						})( btn, curr_obj.action, curr_obj.type === 'file' );
					}
					if (curr_obj.setup)
					{
						curr_obj.setup ( btn );
	...
	function _makeUIMainView ( UI, app ) {
		...
		// Better first-run empty state for both desktop and web builds.
		var ttmp = d.createElement('div');
		ttmp.className = 'pk_tmpMsg pk_emptystate';
		ttmp.innerHTML = (app.isDesktopApp ? (
			'<div class="pk_empty_badge">Desktop Editor</div>' +
			'<img class="pk_empty_logo" src="img/logo.svg" alt="RECwerk" />' +
			'<div class="pk_empty_title">Open audio and start editing</div>' +
			'<div class="pk_empty_copy">Drop a file into the editor, open one from your computer, or load the bundled sample to test the RECwerk workflow immediately.</div>' +
			'<div class="pk_empty_actions">' +
				'<button class="pk_empty_btn pk_empty_btn_primary" data-action="load-local">Load From Computer</button>' +
				'<button class="pk_empty_btn" data-action="load-sample">Load Sample</button>' +
			'</div>' +
			'<div class="pk_empty_meta">Drag and drop WAV, MP3, FLAC, OGG, AIFF and other supported files directly into the window.</div>'
		) : (
			'<div class="pk_empty_badge">Browser Build</div>' +
			'<img class="pk_empty_logo" src="img/logo.svg" alt="RECwerk" />' +
			'<div class="pk_empty_title">RECwerk in the browser</div>' +
			'<div class="pk_empty_copy">This web build is for quick waveform editing, restoration tests, and playback. Open a local file, drop audio onto the page, or launch the bundled sample to see the editor in action.</div>' +
			'<div class="pk_empty_actions">' +
				'<button class="pk_empty_btn pk_empty_btn_primary" data-action="load-local">Open Audio File</button>' +
				'<button class="pk_empty_btn" data-action="load-sample">Load Sample</button>' +
				'<a class="pk_empty_btn pk_empty_btn_link" href="https://github.com/flarkflarkflark/RECwerk/releases/latest" target="_blank" rel="noopener noreferrer">Desktop Downloads</a>' +
			'</div>' +
			'<div class="pk_empty_meta">Use the desktop release for recording, native file handling, and the full RECwerk workflow.</div>'
		));
		main_audio_view.appendChild( ttmp );

		var empty_load_local = ttmp.querySelector('[data-action="load-local"]');
		var empty_load_sample = ttmp.querySelector('[data-action="load-sample"]');
		if (empty_load_local) {
			empty_load_local.onclick = function () {
				UI.fireEvent('RequestLoadLocalFile');
			};
		}
		if (empty_load_sample) {
			empty_load_sample.onclick = function () {
	...
	function _makeUIToolbar (UI) {
		...
		btn_pause.onclick = function() {
			UI.fireEvent('RequestPauseToggle');
			this.blur();
		};
		...
		UI.listenFor('DidSetLoop', function( val ) {
            console.log("UI received DidSetLoop:", val);
		...
		dragNDrop( d.getElementById('app'), 'pk_overlay', function ( e, fileName, filePath ) {
			PKAudioEditor.engine.LoadArrayBuffer ( new Blob([e]), fileName, filePath );
		}, 'arrayBuffer' );
