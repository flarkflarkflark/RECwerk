		...
		var FXBank = {
				...
				return {
					filter : function ( audio_ctx, destination, source, duration ) {
						...
						var sensitivity = val.threshold || 0.3;
						var newBuffer = audio_ctx.createBuffer(buffer.numberOfChannels, buffer.length, buffer.sampleRate);
						
						// Calibratie: Minder agressief dan voorheen
						var maxThreshold = 0.500;
						var minThreshold = 0.005;
						
						for (var c = 0; c < buffer.numberOfChannels; c++) {
							var input = buffer.getChannelData(c);
							var output = newBuffer.getChannelData(c);
							output.set(input);
							...
							var curve = Math.pow(1.0 - sensitivity, 2.0);
							var baseThreshold = minThreshold + (maxThreshold - minThreshold) * curve;
							var levelFactor = Math.max(0.1, Math.min(2.0, rms * 2.0 + 0.1));
							var adaptiveThreshold = baseThreshold * levelFactor;

							for (var i = 10; i < input.length - 10; i++) {
								var secondDeriv = Math.abs(input[i] - 2.0 * input[i-1] + input[i-2]);
								
								if (secondDeriv > adaptiveThreshold) {
									// MAD check: is het een eenzame piek of drukke muziek?
									var mad = 0;
									for(var m=1; m<=8; m++) mad += Math.abs(input[i] - input[i+m]);
									mad /= 8;
									
									// Als MAD te hoog is, is het waarschijnlijk muziek/transient
									if (mad < (rms * 0.5 + 0.01)) {
										var start = i - 2;
										var end = i + 2;
										
										// Controlepunten middelen voor stabiliteit
										var p0 = (input[start-2] + input[start-1]) / 2;
										var p1 = input[start];
										var p2 = input[end];
										var p3 = (input[end+1] + input[end+2]) / 2;
										
										for (var j = start; j <= end; j++) {
											var t = (j - start) / (end - start || 1);
											var t2 = t * t;
											var t3 = t2 * t;
											output[j] = (-0.5*p0 + 1.5*p1 - 1.5*p2 + 0.5*p3)*t3 + (p0 - 2.5*p1 + 2.0*p2 - 0.5*p3)*t2 + (-0.5*p0 + 0.5*p2)*t + p1;
										}
										i = end + 2;
									}
								}
