		...
		var FXBank = {
				...
				return {
					filter : function ( audio_ctx, destination, source, duration ) {
						...
						// Veel conservatievere drempels om flatlines te voorkomen
						var maxThreshold = 0.800;
						var minThreshold = 0.050;
						...
						for (var c = 0; c < buffer.numberOfChannels; c++) {
							...
							var adaptiveThreshold = baseThreshold * (rms + 0.1);

							for (var i = 10; i < input.length - 10; i++) {
								var secondDeriv = Math.abs(input[i] - 2.0 * input[i-1] + input[i-2]);
								
								// Alleen ingrijpen bij extreme acceleratie (tik)
								if (secondDeriv > adaptiveThreshold) {
									// MAD check: is dit een eenzame tik?
									var mad = 0;
									for(var m=1; m<=5; m++) mad += Math.abs(input[i] - input[i+m]);
									mad /= 5;
									
									// CORRECTIE: Als de afwijking (mad) GROOT is ten opzichte van de buren, 
									// dan is het een tik. Alleen dan repareren we!
									if (mad > (rms * 0.2 + 0.05)) {
										var start = i - 1;
										var end = i + 1;
										
										var p0 = input[start-1];
										var p1 = input[start];
										var p2 = input[end];
										var p3 = input[end+1];
										...
										for (var j = start; j <= end; j++) {
											...
											output[j] = p1 + (p2 - p1) * t; // Veilige lineaire interpolatie voor korte tikken
										}
										i = end + 1;
									}
								}
